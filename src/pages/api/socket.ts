...
const UserToken = process.env.NODE_ENV;
...
async function startAnalyze(msg: string, socket: Socket){
  ...
  try {
    ...
    await resp.json();
    socket.emit('analyzeProgress', 'Project Creation')
    console.log('created new project');

    await cloneRepo(data.url, tmpFolder);
    socket.emit('analyzeProgress', "Repo Cloned")
    console.log('cloned the repo')

    await analyzeCode(tmpFolder);
